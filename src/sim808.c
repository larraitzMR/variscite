...
void gprs_at_enter_pin(void){
	...
	bzero(&at_cmd,255);
	sprintf(at_cmd, "AT+CPIN=%s\r",GPRS_SIM_PIN);
...
void gps_power_on(int auto_report){
	...
	if (auto_report){
		...
		result = uart_write_buffer(fd_gprs, at_cmd_manual, strlen(at_cmd_manual));
	}
...
int gprs_send_position( t_position lat, t_position lon){
	...
	//Checksum
	frame_buffer[idx_input].data[9] = gprs_checksum((unsigned char *)frame_buffer[idx_input].data,9);
	...
	if (idx_input == idx_output) gprs_buffer_full=1;
...
void gprs_send_frame_in_buffer(void){
	...
	result = uart_write_buffer(fd_gprs, frame_buffer[idx_output].data, frame_buffer[idx_output].len);
